...
def _id(h):
    ...
    return {"volumeSerial":int(x.VolumeSerialNumber),"fileId":bytes(x.FileId.Identifier).hex(),"creationTime":creation}
def _id64(h):
    x=BHFI()
    ...
    while True:
        ...
        while True:
            ...
            if name not in (".",".."):
                ...
                out.append((name,fid,int(r.ct)&((1<<64)-1),int(r.attrs)))
            nxt=int(r.Next)
            if not nxt: break
...
def _delete_children(parent):
    pvol,_=_id64(parent)
    for name,fid,created,listed in _entries(parent):
        if listed&REPARSE: raise WindowsCleanupError(f"reparse child denied: {name}")
        h=_open_relative(parent,name)
        try:
            attrs=_attrs(h);vol,opened=_id64(h);basic=BASIC()
            if not K.GetFileInformationByHandleEx(h,0,ctypes.byref(basic),ctypes.sizeof(basic)): _err("cannot read child creation identity")
            if attrs&REPARSE: raise WindowsCleanupError(f"reparse child denied: {name}")
            if vol!=pvol or opened!=fid or (int(basic.c)&((1<<64)-1))!=created: raise WindowsCleanupError(f"cleanup child identity changed: {name}")
...
def delete_tree_exact(path:str|os.PathLike[str],expected_native:dict)->None:
    _win()
    if not isinstance(expected_native,dict) or set(expected_native)!={"volumeSerial","fileId","creationTime"}: raise WindowsCleanupError("missing native generation identity")
    try:vol=int(expected_native["volumeSerial"]);fid=str(expected_native["fileId"]).lower();creation=int(expected_native["creationTime"])
    except Exception as e: raise WindowsCleanupError("invalid native generation identity") from e
    if vol<0 or creation<=0 or len(fid)!=32 or any(c not in "0123456789abcdef" for c in fid): raise WindowsCleanupError("invalid native generation identity")
    ...
    try:
        ...
        if obs!={"volumeSerial":vol,"fileId":fid,"creationTime":creation}: raise WindowsCleanupError("workspace generation identity changed before cleanup")
