...
import argparse, hashlib, json, os, socketserver, subprocess, sys, threading, time
...
MUTATING_METHODS={"task.create","workspace.claim","workspace.heartbeat","workspace.release"}
...
def _activation_probe_enabled():
    return bool(os.environ.get("FORGEBOSS_PROBE_ENDPOINT") or os.environ.get("FORGEBOSS_PROBE_CHALLENGE") or os.environ.get("FORGEBOSS_ACTIVATION_NONCE"))
...
def _run_candidate_selftests():
    ...
    try:
        cp=subprocess.run([sys.executable,"-m","unittest","forgeboss.control.test_known_good","forgeboss.control.test_activation"],cwd=str(ROOT),env=env,stdin=subprocess.DEVNULL,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL,timeout=90,check=False)
    except Exception:return False
    return cp.returncode==0

def _candidate_multiagent_check(store):
    required=("create_task","claim_workspace","heartbeat","release","assert_writer","snapshot")
    if any(not callable(getattr(store,name,None)) for name in required):return False
    try:snap=store.snapshot()
    except Exception:return False
    return isinstance(snap,dict) and all(k in snap for k in ("tasks","leases","workers","lastEventSeq"))

def _submit_candidate_activation_probe(daemon):
    ...
    with _PROBE_SUBMIT_LOCK:
        ...
        try:fresh_proc=process_identity(os.getpid());state=daemon.activation.status();snap=daemon.store.snapshot()
        ...
        control=isinstance(snap,dict) and int(snap.get("schemaVersion",-1))==3
        selftests=_run_candidate_selftests()
        multiagent=_candidate_multiagent_check(daemon.store)
...
class ForgeBossDaemon:
    def __init__(self):
        ...
        self.store=ControlStore(DB);self.secret_path,self.secret=secret_file(ROOT)
...
def main():
    ...
    with Server((ns.host,ns.port),Handler) as srv:
        if _activation_probe_enabled():_submit_candidate_activation_probe(DAEMON)
        print(json.dumps({"forgebossd":"ready","host":ns.host,"port":ns.port,"db":str(DB),"controllerIdentity":DAEMON.identity,"controllerProcessIdentity":DAEMON.processIdentity}),flush=True);srv.serve_forever()
