...
def sample(key_id="k1", branch=True):
    a={
        ...
        "runtime":{"adapter":"mini-swe","provider":"openai","model":"gpt-5.6"},
        ...
        "issuedAt":100.0,
        "expiresAt":200.0,
        "controllerKnownGood":{"revision":"d"*40,"manifestSha256":"e"*64,"identitySha256":"f"*64},
        "reviewPolicy":{"independentReviewRequired":True,"authorMayReview":False,"reviewerId":"worker-b"},
    }
...
class SchemaTests(unittest.TestCase):
    ...
    def test_known_good_revision_is_git_sha_not_integer(self):
        validate_authority(sample())
        a=sample();a["controllerKnownGood"]["revision"]=1
        with self.assertRaises(ValueError):validate_authority(a)
    ...
    def test_scope_hostile_windows_forms(self):
        bads=("C:/src/a.py","/src/a.py","//server/share/a.py","src\\a.py","src/a.py.","src/a.py ","src/a.py:ads","NUL.txt","src//a.py","src/../a.py")
        for bad in bads:
            a=sample();a["allowedPaths"]=[bad]
            with self.subTest(bad=bad),self.assertRaises(ValueError):validate_authority(a)
        a=sample();a["allowedPaths"]=["src/a.py"];a["deniedPaths"]=["SRC"]
        with self.assertRaises(ValueError):validate_authority(a)
    def test_worktree_canonical_and_root_bound(self):
...
class TrustTests(unittest.TestCase):
    def test_rotation_and_generation(self):
        cur=key("k1",90,150,180,b"a");nxt=key("k2",160,None,300,b"b")
        t=PinnedAuthorityTrust(3,(cur,nxt),"k1","k2",minimum_generation=3)
    ...
    def test_durable_monotonic_updates(self):
        with tempfile.TemporaryDirectory() as td:
            p=Path(td)/"trust.json"
            t3=PinnedAuthorityTrust(3,(key(byte=b"a"),),"k1",minimum_generation=3)
            write_pinned_trust(p,t3)
            before=p.read_bytes()
            ...
            self.assertEqual(write_pinned_trust(p,t3),p)
            ...
            t4=PinnedAuthorityTrust(4,(key(byte=b"b"),),"k1",minimum_generation=4)
            lock=p.parent/f".{p.name}.update.lock";lock.write_text("other writer\n",encoding="utf-8")
            with self.assertRaises(RuntimeError):write_pinned_trust(p,t4)
            self.assertEqual(before,p.read_bytes());self.assertTrue(lock.exists());lock.unlink()
            write_pinned_trust(p,t4)
            self.assertEqual(load_pinned_trust(p,minimum_generation=4).generation,4)
            with self.assertRaises(PermissionError):load_pinned_trust(p,minimum_generation=5)
...
class CryptoTests(unittest.TestCase):
    ...
    def packet(self,a):
        return {"authority":a,"signature":"ed25519:"+base64.b64encode(self.private.sign(canonical(a))).decode()}
    def test_valid_tamper_and_legacy(self):
        a=sample();v=ControllerAuthorityVerifier(self.trust,worktree_root="C:/work")
        self.assertEqual(v.verify(self.packet(a),110)["currentHead"],"b"*40)
    ...
    def test_branchless_signature(self):
        a=sample(branch=False);v=ControllerAuthorityVerifier(self.trust,worktree_root="C:/work")
        self.assertNotIn("branch",v.verify(self.packet(a),110))
    def test_signer_nested_mutation_denied(self):
        a=sample()
        def mutate(req):
            req["authority"]["runtime"]["model"]="evil"
            return {"authority":req["authority"],"signature":"ed25519:x"}
        with self.assertRaises(PermissionError):
            ControllerAuthoritySignerClient(mutate,"controller-1",worktree_root="C:/work").sign_worker_launch(a)
