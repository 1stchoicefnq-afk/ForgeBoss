...
class StrictAuthoritySchemaTests(unittest.TestCase):
    ...
    def test_rejects_path_tool_collisions_and_allow_deny_overlap(self):
        authority = sample()
        authority["allowedPaths"] = ["SRC/a.py", "src/a.py."]
        with self.assertRaises(ValueError):
            validate_authority(authority)
        authority = sample()
        authority["deniedPaths"] = ["SRC/A.PY"]
        with self.assertRaises(ValueError):
            validate_authority(authority)
        authority = sample()
    ...
    def test_rejects_bad_review_policy(self):
        authority = sample()
...
class AuthorityCryptoTests(unittest.TestCase):
    ...
    def test_signer_rejects_mutated_packet(self):
        authority = sample()
        client = ControllerAuthoritySignerClient(
            lambda request: {"authority": dict(request["authority"], attempt=2), "signature": "ed25519:x"},
            "controller-1",
        )
        with self.assertRaises(PermissionError):
            client.sign_worker_launch(authority)
