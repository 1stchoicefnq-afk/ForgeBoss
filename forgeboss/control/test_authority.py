...
class SchemaTests(unittest.TestCase):
    ...
    def test_scope_hostile_windows_forms(self):
        ...
        with self.assertRaises(ValueError):validate_authority(a)
    def test_windows_short_name_alias_candidates_rejected(self):
        for bad in ("src/PROGRA~1/a.py","src/TESTFI~2.JS","tests/A~1/test.js","src/file~name.py"):
            a=sample();a["allowedPaths"]=[bad]
            with self.subTest(bad=bad),self.assertRaises(ValueError):validate_authority(a)
        a=sample();a["deniedPaths"]=["PRIVATE~1"]
        with self.assertRaises(ValueError):validate_authority(a)
        validate_authority(sample())
    def test_worktree_lexical_root_bound(self):
        validate_authority(sample(),worktree_root="C:/work")
        for bad in ("relative/work","c:/work/one","C:\\work\\one","C:/work/../one","C:/work/one.","C:/work/COM¹","C:/work/LPT².txt","C:/work/CONIN$","C:/work/conout$","C:/work/PROGRA~1","C:/work/LONGNA~2/child"):
