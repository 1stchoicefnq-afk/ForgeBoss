...
def _scope_overlap(a,b):
    # Store has one allowedPaths authority list rather than scheduler's separate files/write_roots.
    # Treat each authority conservatively as hierarchical: siblings remain disjoint, while exact
    # and ancestor/descendant authorities collide. This covers scheduler file/root/root semantics
    # without under-authorizing concurrent writers.
    for x in a:
        for y in b:
...
class ControlStore:
    ...
    def create_task(self,t):
        now=time.time();_scope_authorities(t.get("allowedPaths",[]))
        with self._lock:
    ...
    def _live_cross_task_conflicts_locked(self,task_id,worktree,scope,now):
        worktree_id=_physical_worktree_identity(worktree)
        rows=self.db.execute("""SELECT wl.task_id,wl.worktree_path,t.allowed_paths_json
    ...
    def claim_workspace(self,task_id,run_id,worktree,branch,current_head,ttl_seconds=1200,runtime_id=None,worktree_root=None,budget_reserved=0.0):
        ...
        worktree=canonical_worktree_path(worktree,worktree_root);ttl_seconds=_ttl(ttl_seconds);now=time.time()
        with self._lock:
            begun=False
            try:
                ...
                self._live_cross_task_conflicts_locked(task_id,worktree,scope,now)
                ...
                # First update reserved the budget under old revision without incrementing it; this second rowcount gate closes claim mutation.
    ...
    def heartbeat(self,task_id,run_id,owner_epoch,ttl_seconds=1200,current_head=None,expected_head=None):
        ttl_seconds=_ttl(ttl_seconds);now=time.time()
        with self._lock:
            begun=False
            try:
                self.db.execute("BEGIN IMMEDIATE");begun=True
