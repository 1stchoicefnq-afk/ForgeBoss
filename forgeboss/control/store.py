    ...
    def _migrate(self):
        with self._lock:
            ...
            for name,definition in (
                ...
                ("assignment_sha256","TEXT"),("retry_reason","TEXT")):
        ...
        with self._lock:
            ...
            try:
                ...
                cur=self.db.execute("""UPDATE tasks SET budget_run_id=?,assigned_builder_id=?,assignment_generation=?,assignment_token_hash=?,assignment_sha256=?,
                    revision=revision+1,updated_at=? WHERE task_id=? AND revision=?""",(budget_run_id,builder_id,generation,token_hash,digest,now,task_id,int(task["revision"])))
