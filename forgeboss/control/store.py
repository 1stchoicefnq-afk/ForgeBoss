...
class ControlStore:
    ...
    def _live_cross_task_conflicts_locked(self,task_id,repository,worktree,scope,now):
        worktree_id=_physical_worktree_identity(worktree)
        rows=self.db.execute("""SELECT wl.task_id,wl.worktree_path,t.repository,t.allowed_paths_json
        ...
        for row in rows:
            ...
            if str(row["repository"])!=str(repository):continue
    ...
    def claim_workspace(self,task_id,run_id,worktree,branch,current_head,ttl_seconds=1200,runtime_id=None,worktree_root=None,budget_reserved=0.0):
        ...
        with self._lock:
            ...
            try:
                ...
                self._live_cross_task_conflicts_locked(task_id,task["repository"],worktree,scope,now)
