...
def _write_record(path: Path, record: dict) -> None:
    record = dict(record)
    record["version"] = QUARANTINE_VERSION
...
def _path_identity(path: Path, code: str = "WORKSPACE_IDENTITY_INVALID") -> dict:
    ...
    return {
        ...
        "ctimeNs": int(getattr(st, "st_ctime_ns", int(st.st_ctime * 1_000_000_000))),
        "mode": int(st.st_mode),
    }
...
def _identity_matches(path: Path, expected: dict | None) -> bool:
    ...
    for key in ("dev", "ino", "mode"):
        if int(observed.get(key, -1)) != int(expected.get(key, -2)):
            return False
    return True
...
def quarantine_status(
    workspace: str | os.PathLike[str], workspace_root: str | os.PathLike[str]
) -> dict | None:
...
def discover_quarantined_workspaces(workspace_root: str | os.PathLike[str]) -> list[dict]:
    ...
    for stage in sorted(root.glob(f"{STAGE_PREFIX}*")):
        if str(stage) in referenced_stages:
            continue
        if stage.is_symlink() or _is_reparse(stage):
            records.append({"version": QUARANTINE_VERSION, "state": "orphan-untrusted", "stage": str(stage), "target": None})
            continue
        records.append({"version": QUARANTINE_VERSION, "state": "orphan-untracked", "stage": str(stage), "target": None})
    return records
...
def reconcile_quarantined_workspace(
    workspace: str | os.PathLike[str],
    workspace_root: str | os.PathLike[str],
    generation_id: str,
) -> dict:
    ...
    if record["generation"] != generation_id:
        raise WorkspaceProvisionError(
            ...
            "quarantine generation does not match requested cleanup generation",
            generation_id=record["generation"],
        )
    stage = Path(record["stage"])
    ...
    if existing.exists() or existing.is_symlink() or _is_reparse(existing):
        ...
        try:
            ...
            raise WorkspaceProvisionError(
                ...
                f"quarantined workspace cleanup failed: {ex}",
                cleanup_code=type(ex).__name__.upper(),
                generation_id=generation_id,
            ) from ex
...
def verify_workspace(
    ...
    *,
    ...
    _allow_generation: str | None = None,
) -> WorkspaceIdentity:
    git = _git_executable(git_executable)
    root = _canonical_existing_dir(workspace_root, "WORKSPACE_ROOT_INVALID")
...
def cleanup_workspace(
    ...
    if found is not None:
        raise WorkspaceProvisionError(
            "WORKSPACE_QUARANTINED",
            "quarantined workspaces require generation-bound reconciliation",
            generation_id=found[1]["generation"],
        )
    if not candidate.exists() and not candidate.is_symlink() and not _is_reparse(candidate):
        return False
    _assert_no_link_components(candidate, "WORKSPACE_CLEANUP_DENIED")
...
def _provision_failure_cleanup(
    root: Path,
    target: Path,
    stage: Path,
    record_path: Path,
    ...
    try:
        if candidate.exists() or candidate.is_symlink() or _is_reparse(candidate):
            _assert_no_link_components(candidate, "WORKSPACE_CLEANUP_DENIED")
            _assert_plain_existing_path(candidate, "WORKSPACE_CLEANUP_DENIED")
            if not _identity_matches(candidate, record.get("identity")):
    ...
    try:
        _write_record(record_path, quarantined)
    except Exception as record_error:
        raise WorkspaceProvisionError(
            "WORKSPACE_QUARANTINE_WRITE_FAILED",
            ...
            provision_code=_exception_code(provision_error),
            cleanup_code=_exception_code(cleanup_error),
            generation_id=record["generation"],
        ) from record_error
    raise WorkspaceProvisionError(
        "WORKSPACE_PROVISION_CLEANUP_FAILED",
...
def provision_workspace(
    ...
    target_hash = hashlib.sha256(str(target).encode("utf-8")).hexdigest()[:16]
    stage = root / f"{STAGE_PREFIX}{target_hash}-{generation}"
    record_path = _record_path(root, target)
    record = {
    ...
    try:
        ...
        _fsync_dir(root)
        record["identity"] = _path_identity(stage)
        ...
        stage.rename(target)
        _fsync_dir(root)
        if not _identity_matches(target, record["identity"]):
            raise WorkspaceProvisionError("WORKSPACE_GENERATION_MISMATCH", "workspace identity changed across final rename")
        record["state"] = "finalizing"
        _write_record(record_path, record)
        identity = verify_workspace(
            target,
            root,
            requested,
            branch,
