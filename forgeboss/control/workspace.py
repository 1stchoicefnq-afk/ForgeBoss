...
def _read_record(path: Path) -> dict:
    ...
    if value["state"] not in {"preparing", "provisioning", "renaming", "finalizing", "quarantined"}:
...
def _generation_continuity_matches(path: Path, expected: dict | None) -> bool:
...
def _provision_failure_cleanup(root: Path, target: Path, stage: Path, record_path: Path, record: dict, provision_error: BaseException) -> None:
