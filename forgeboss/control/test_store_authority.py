...
class StoreAuthorityV1Tests(unittest.TestCase):
    def setUp(self):
    ...
    def claim(self,tid,run,assignment,amount,store=None,global_revision=None):
        s=store or self.store
        if global_revision is None:global_revision=int(s.get_budget_run(assignment["budgetRunId"])["revision"])
        return s.claim_workspace(tid,run,self.root/tid,"b-"+tid,"a"*40,runtime_id="mini-swe",worktree_root=self.root,budget_reserved=amount,
    ...
    def test_release_and_revoke_do_not_refund(self):
        ...
        lease2=self.claim("A","rb",a,2);self.store.revoke_writer("A","rb",lease2["owner_epoch"],"stop")
    ...
    def test_retry_required_for_revoked_and_cancelled(self):
        ...
        self.store.retry_task("A","replacement");b=self.store.assign_builder("A","builder-b","GLOBAL");self.claim("A","rb",b,1)
    ...
    def test_restart_preserves_budget_assignment_revoke_and_attempt(self):
        ...
        self.store.db.close();self.store=ControlStore(self.db)
        self.assertEqual(self.store.get_budget_run("GLOBAL")["reserved_exact"],"2");self.assertEqual(self.store.get_task("A")["assignment_sha256"],a["assignmentSha256"])
