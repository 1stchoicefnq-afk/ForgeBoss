...
def _branch(value):
    ...
    if b!=b.strip() or b.startswith(("/", ".")) or b.endswith(("/",".",".lock")) or "\\" in b or ".." in b or "//" in b or "@{" in b or any(ch.isspace() for ch in b) or any(ch in b for ch in "~^:?*["):
...
class ControllerAuthorityVerifier:
    def __init__(self,trust,worktree_root=None):
        if not isinstance(trust,PinnedAuthorityTrust): raise TypeError("pinned authority trust required")
        self._trust=trust; self._worktree_root=worktree_root
    def verify(self,packet,now):
        ...
        a=copy.deepcopy(packet["authority"]); validate_authority(a,now,self._worktree_root)
...
class ControllerAuthoritySignerClient:
    def __init__(self,transport,controller_identity,worktree_root=None):
        if not callable(transport): raise TypeError("signer transport must be callable")
        self._transport=transport; self._controller_identity=_strict_str(controller_identity,"controller identity",128,_ID); self._worktree_root=worktree_root
    def sign_worker_launch(self,authority):
        if not isinstance(authority,dict): raise ValueError("authority must be object")
        checked=copy.deepcopy(authority); validate_authority(checked,worktree_root=self._worktree_root); expected=canonical(checked)
        ...
        try:
            returned=copy.deepcopy(res["authority"]); validate_authority(returned,worktree_root=self._worktree_root)
