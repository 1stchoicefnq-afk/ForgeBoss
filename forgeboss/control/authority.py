...
def _branch(value):
    ...
    bad_chars = set(" ~^:?*[\\")
    if (
        any(ch in bad_chars for ch in branch)
...
def _worktree(value):
    return _strict_str(value, "worktreePath", max_len=2048)
...
def validate_authority(authority, now=None):
    ...
    _strict_str(authority["repository"], "repository", max_len=201, pattern=_REPO)
...
class ControllerAuthorityVerifier:
    ...
    def verify(self, packet, now):
        ...
        authority = validate_authority(packet["authority"], now)
...
class ControllerAuthoritySignerClient:
    ...
    def sign_worker_launch(self, authority):
        ...
        request = {"type": "sign-worker-launch", "controllerIdentity": self._controller_identity, "authority": checked}
        response = self._transport(request)
        if not isinstance(response, dict) or set(response) != {"authority", "signature"} or response["authority"] != checked:
