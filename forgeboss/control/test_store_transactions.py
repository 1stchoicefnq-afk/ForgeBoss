...
class StoreTransactionCollisionTests(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        self.td.cleanup()
    def task(self,task_id,paths,budget=2.0):
        return self.store.create_task({"taskId":task_id,"repository":"1stchoicefnq-afk/ForgeBoss","purpose":"test","baseSha":"a"*40,"branch":"b-"+task_id,"allowedPaths":list(paths),"requiredTests":[],"budgetUsd":budget})
    ...
    def test_same_physical_worktree_different_tasks_is_rejected_before_budget_or_run_mutation(self):
        ...
        self.claim(self.store,"A","run-a",path)
    ...
    def test_windows_equivalent_worktree_case_and_trailing_dot_collide(self):
        ...
        self.claim(self.store,"A","run-a",self.root/"Worker")
    ...
    def test_overlapping_scope_across_distinct_worktrees_is_rejected(self):
        self.task("A",["src/"]);self.task("B",["SRC/child/file.py"])
    ...
    def test_windows_equivalent_scope_spelling_collides(self):
        self.task("A",[r"Src\Child. \file.py"]);self.task("B",["src/child/file.py"])
        self.claim(self.store,"A","run-a",self.root/"wa")
    ...
    def test_concurrent_same_physical_worktree_has_exactly_one_winner_and_loser_leaks_nothing(self):
        self.task("A",["src/a.py"]);self.task("B",["src/b.py"]);path=self.root/"shared";barrier=threading.Barrier(2)
        s1=ControlStore(self.db);s2=ControlStore(self.db)
        ...
        try:
            with concurrent.futures.ThreadPoolExecutor(max_workers=2) as pool:
                results=list(pool.map(lambda args:attempt(*args),[(s1,"A","run-a"),(s2,"B","run-b")]))
    ...
    def test_concurrent_overlapping_scope_has_exactly_one_winner(self):
        ...
        try:
            with concurrent.futures.ThreadPoolExecutor(max_workers=2) as pool:
                results=list(pool.map(lambda args:attempt(*args),[(s1,"A","run-a",self.root/"wa"),(s2,"B","run-b",self.root/"wb")]))
        finally:s1.db.close();s2.db.close()
    ...
    def test_duplicate_run_id_failure_rolls_back_budget_lease_and_claim_event(self):
        self.task("A",["src/a.py"]);self.task("B",["src/b.py"]);self.claim(self.store,"A","same-run",self.root/"wa")
        before=self.counts("B")
