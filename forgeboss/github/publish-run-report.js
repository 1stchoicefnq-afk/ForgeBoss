...
(async()=>{
 let r=await fetch(`https://api.github.com/app/installations/${iid}/access_tokens`,{method:'POST',headers:{Authorization:`Bearer ${jwt}`,Accept:'application/vnd.github+json','X-GitHub-Api-Version':'2022-11-28','Content-Type':'application/json'},body:JSON.stringify({repositories:[repo],permissions:{issues:'write',pull_requests:'read',contents:'read'}})});
 if(!r.ok)throw new Error(`installation token HTTP ${r.status}: `+await r.text());const tok=(await r.json()).token;
 r=await fetch(`https://api.github.com/repos/${owner}/${repo}/issues/${pr}/comments`,{method:'POST',headers:{Authorization:`Bearer ${tok}`,Accept:'application/vnd.github+json','X-GitHub-Api-Version':'2022-11-28','Content-Type':'application/json'},body:JSON.stringify({body:fs.readFileSync(md,'utf8')})});
