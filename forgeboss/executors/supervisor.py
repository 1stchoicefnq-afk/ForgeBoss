...
def run_supervised(argv: Sequence[str], authority_check: Callable[[], bool], *, cwd=None,
    ...
    kwargs={"cwd":os.fspath(cwd) if cwd is not None else None,"env":dict(env) if env is not None else None,
            "stdin":subprocess.DEVNULL,"stdout":subprocess.DEVNULL,"stderr":subprocess.DEVNULL,"text":True,"shell":False}
    ...
    proc=subprocess.Popen(list(argv),**kwargs); pgid=proc.pid if os.name != "nt" else None
    try:
        if os.name == "nt":
            job=_WindowsJob()
            try: job.assign(proc.pid)
            except Exception:
                proc.kill(); proc.wait(timeout=2); job.close(); raise
