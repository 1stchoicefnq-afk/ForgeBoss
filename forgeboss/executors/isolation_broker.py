...
def _capture_file(path: Path):
    if not path.exists():
        return {"state": {"kind": "missing"}, "backup": None}
    if guard.is_linklike(path) or not path.is_file():
        return {"state": {"kind": "unsafe"}, "backup": None}
    data = path.read_bytes()
    return {"state": {"kind": "file", "sha256": _sha256(data), "size": len(data)}, "backup": data}


def _file_state(path: Path):
    return _capture_file(path)["state"]


def _capture_allowed_preimages(host: Path, packet: dict):
    allowed, _ = guard.validate_packet(packet)
    out = {}
    for rel in allowed:
        guard.assert_paths_contained(host, [rel])
        target = host / rel
        captured = _capture_file(target)
        if captured["state"].get("kind") == "unsafe":
            raise IsolationBrokerError("host target is unsafe before broker run: " + rel)
        out[rel.casefold()] = {"rel": rel, "target": target, "state": captured["state"], "backup": captured["backup"]}
    return out


def _capture_authority(host: Path):
    return {"ordinary": guard.snapshot(host), "git": guard.git_metadata_snapshot(host), "head": guard.git(host, "rev-parse", "HEAD")}
...
def _stage_changes(host: Path, packet: dict, changes, preimages):
    allowed, _ = guard.validate_packet(packet)
    ...
    for item in changes:
        ...
        seen.add(key)
        bound = preimages.get(key)
        if not bound or bound["rel"] != allowed_keys[key]:
            raise IsolationBrokerError("missing pre-run target binding: " + rel)
        ...
        staged.append({"rel": bound["rel"], "target": bound["target"], "action": action, "data": data, "preimage": bound["state"], "backup": bound["backup"]})
    return staged
...
def _expected_post_snapshot(baseline: dict, staged) -> dict:
    ...
    for item in staged:
        if item["action"] == "delete":
            expected.pop(item["rel"], None)
        else:
            data = item["data"]
            expected[item["rel"]] = {"kind": "file", "sha256": _sha256(data), "size": len(data)}
    return expected
...
def _apply_changes(host: Path, packet: dict, authority_baseline: dict, changes, preimages) -> list[str]:
    staged = _stage_changes(host, packet, changes, preimages)
...
def run_isolated_mini_swe(lease_path, token, packet_path, workspace, control_envelope, cli_budget, model_name, image):
    ...
    preimages = _capture_allowed_preimages(host, packet)
    request = {
        "schema": 1,
    ...
    applied = _apply_changes(host, packet, host_authority, reply.get("changes", []), preimages)
