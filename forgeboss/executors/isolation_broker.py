...
def _casefold_state_map(mapping, label: str):
    if not isinstance(mapping, dict):
        raise IsolationBrokerError(label + " must be an object")
    out = {}
    for raw_path, state in mapping.items():
        if not isinstance(raw_path, str):
            raise IsolationBrokerError(label + " path key must be text")
        rel = guard.norm(raw_path)
        folded = rel.casefold()
        if folded in out:
            raise IsolationBrokerError(label + " contains Windows case collision: " + rel)
        out[folded] = (rel, state)
    return out
...
def _casefold_allowed(allowed):
    ...
    for raw_path in allowed:
        ...
        folded = rel.casefold()
        if folded in out:
            raise IsolationBrokerError("allowed paths contain Windows case collision: " + rel)
        out[folded] = rel
    return out
...
def _verify_receipt(reply: dict, request: dict, host: Path, packet: dict, pre_authority: dict, pre_targets: dict):
    ...
    allowed, _ = guard.validate_packet(packet)
    allowed_keys = _casefold_allowed(allowed)
    pre_map = _casefold_state_map(pre_targets, "preTargets")
    if set(pre_map) != set(allowed_keys):
        raise IsolationBrokerError("preTargets identity differs from allowed path set")
    applied = receipt.get("appliedPaths")
    if not isinstance(applied, list):
        raise IsolationBrokerError("reintegration receipt applied path set is invalid")
    applied_folded = []
    seen = set()
    for raw_path in applied:
        if not isinstance(raw_path, str):
            raise IsolationBrokerError("reintegration receipt applied path set is invalid")
        rel = guard.norm(raw_path)
        folded = rel.casefold()
        if folded not in allowed_keys or folded in seen:
            raise IsolationBrokerError("reintegration receipt applied path set is invalid")
        seen.add(folded);applied_folded.append(folded)
    post_map = _casefold_state_map(receipt.get("postTargets"), "postTargets")
    if any(k not in allowed_keys for k in post_map):
        raise IsolationBrokerError("reintegration receipt postTargets contains out-of-scope path")
    canonical_post = {}
    for folded, canonical_rel in allowed_keys.items():
        expected = post_map.get(folded, pre_map[folded])[1]
        ...
        if actual != expected:
            raise IsolationBrokerError("host target does not match authenticated broker receipt: " + canonical_rel)
    ...
    baseline_map = _casefold_state_map(pre_authority.get("ordinary"), "preAuthority ordinary")
    current_map = _casefold_state_map(current.get("ordinary"), "current ordinary")
    expected_map = {k:(display,state) for k,(display,state) in baseline_map.items()}
    for folded, canonical_rel in allowed_keys.items():
        state = canonical_post[canonical_rel]
        if state.get("kind") == "missing":
            expected_map.pop(folded, None)
        else:
            expected_map[folded] = (canonical_rel, state)
    if {k:v[1] for k,v in current_map.items()} != {k:v[1] for k,v in expected_map.items()}:
    ...
    return [allowed_keys[x] for x in applied_folded]
