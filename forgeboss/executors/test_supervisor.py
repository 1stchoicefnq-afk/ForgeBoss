...
class SupervisorContractTests(unittest.TestCase):
    def test_windows_bootstrap_is_stdin_gated_before_exec(self):
        with tempfile.TemporaryDirectory() as td:
            marker=Path(td)/"ran"
            p=subprocess.Popen([sys.executable,"-m","forgeboss.executors.supervisor_child"],stdin=subprocess.PIPE,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL,text=True)
            time.sleep(.1);self.assertFalse(marker.exists());self.assertIsNone(p.poll())
            argv=[sys.executable,"-c",f"open({str(marker)!r},'w').write('x')"]
            p.stdin.write(json.dumps({"argv":argv,"cwd":None,"env":dict(os.environ)})+"\n");p.stdin.flush();p.stdin.close()
            self.assertEqual(p.wait(timeout=3),0);self.assertTrue(marker.exists())
