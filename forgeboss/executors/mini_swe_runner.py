...
def main() -> int:
    ...
    if not lease or not lease_token:
        print("FORGEBOSS SAFE STOP: unified executor lease missing.", file=sys.stderr)
        return 13
    ...
    if governed:
        if not control_envelope:
            print("FORGEBOSS SAFE STOP: governed control envelope missing.", file=sys.stderr)
            return 13
        if not re.fullmatch(r"[^\s@]+@sha256:[0-9a-f]{64}", image_name):
            print(
                "FORGEBOSS SAFE STOP: governed mini-swe image must be digest-pinned.",
                file=sys.stderr,
            )
            return 13
        ...
        if verify.returncode:
            print(
                "FORGEBOSS SAFE STOP: " + (verify.stdout or verify.stderr),
                file=sys.stderr,
            )
            return 13
...
"""
        ...
        with authority as control:
            ...
            run_args.extend(["--network", "none", "--mount", mount])
        ...
        if post.returncode:
            result["error"] = (
                "ForgeBoss postflight denied worker result: "
                + (post.stdout or post.stderr)[-1200:]
            )
            return 13
