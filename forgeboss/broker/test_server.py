...
class GoodVerifier:
    def verify(self,req):return SimpleNamespace(attestation_id="protected-attestation-1")
...
class BrokerServerTests(unittest.TestCase):
    ...
    def test_success_receipt_is_commit_ref_handoff_not_file_payload(self):
        repo=self.work/"private";repo.mkdir();rr=SimpleNamespace(completed=True,cost_usd=.1,calls=2,error=None);hand=SimpleNamespace(base_commit="a"*40,result_commit="b"*40,result_tree="c"*40,result_ref="refs/forgeboss/results/x/y",old_oid="0"*40,new_oid="b"*40,applied_paths=("x.py",),diff_sha256="d"*64,handoff_repo_id="e"*64)
        ...
        with self.patches():env=b.handle(request(self.work))
        signed=env["signedReceipt"];self.assertEqual(signed["changes"],[]);self.assertFalse(signed["hostWorkspaceMounted"]);self.assertFalse(signed["localCopybackRequired"]);self.assertFalse(signed["reintegrationReceipt"]["hostWorktreeAuthoritative"]);self.assertEqual(signed["reintegrationReceipt"]["resultCommit"],"b"*40)
