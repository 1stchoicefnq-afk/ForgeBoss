...
from forgeboss.broker.reintegrate import result_ref

class FakeSigner:
    def sign(self,obj):return {"signedReceipt":obj,"receiptSignature":"test"}
class GoodVerifier:
    def verify(self,req):return SimpleNamespace(attestation_id="protected-attestation-1",result_ref=req["resultRef"])
...
class BrokerServerTests(unittest.TestCase):
    ...
    def test_paid_runtime_failure_returns_signed_truthful_cost_and_calls(self):
        repo=self.work/"private";repo.mkdir();rr=SimpleNamespace(completed=False,cost_usd=.17,calls=4,error="provider failed")
        b=server.BrokerServer(authority_verifier=GoodVerifier(),signer=FakeSigner(),materializer=lambda *_:repo,runner=lambda *_:rr)
    ...
    def test_scope_or_prehash_mismatch_fails_before_protected_authority(self):
        ...
        req=request(self.work);req["preAuthoritySha256"]="0"*64
    ...
    def test_success_receipt_is_commit_ref_handoff_not_file_payload(self):
        repo=self.work/"private";repo.mkdir();rr=SimpleNamespace(completed=True,cost_usd=.1,calls=2,error=None);ref=result_ref("task-1","run-1");hand=SimpleNamespace(base_commit="a"*40,result_commit="b"*40,result_tree="c"*40,result_ref=ref,old_oid="0"*40,new_oid="b"*40,applied_paths=("x.py",),diff_sha256="d"*64,handoff_repo_id="e"*64)
        ...
        with self.patches():env=b.handle(request(self.work))
        signed=env["signedReceipt"];self.assertEqual(signed["changes"],[]);self.assertFalse(signed["hostWorkspaceMounted"]);self.assertFalse(signed["localCopybackRequired"]);self.assertFalse(signed["reintegrationReceipt"]["hostWorktreeAuthoritative"]);self.assertEqual(signed["reintegrationReceipt"]["resultRef"],ref)
    def test_wrong_handoff_result_ref_fails_after_consume(self):
        repo=self.work/"private";repo.mkdir();rr=SimpleNamespace(completed=True,cost_usd=.1,calls=2,error=None);hand=SimpleNamespace(base_commit="a"*40,result_commit="b"*40,result_tree="c"*40,result_ref="refs/forgeboss/results/evil",old_oid="0"*40,new_oid="b"*40,applied_paths=("x.py",),diff_sha256="d"*64,handoff_repo_id="e"*64)
        b=server.BrokerServer(authority_verifier=GoodVerifier(),signer=FakeSigner(),materializer=lambda *_:repo,runner=lambda *_:rr,handoff=lambda *_:hand)
        with self.patches():env=b.handle(request(self.work))
        self.assertFalse(env["signedReceipt"]["completed"]);self.assertIn("result ref differs",env["signedReceipt"]["error"])
