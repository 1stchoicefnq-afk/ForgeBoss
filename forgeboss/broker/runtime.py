...
def _git_env(root:Path):
    home=root/"git-home";home.mkdir(parents=True,exist_ok=True)
    keep=("PATH","SystemRoot","WINDIR","COMSPEC","PATHEXT","TEMP","TMP","TMPDIR","LANG","LC_ALL")
    env={k:os.environ[k] for k in keep if k in os.environ}
    env.update({"HOME":str(home),"USERPROFILE":str(home),"GIT_CONFIG_NOSYSTEM":"1","GIT_CONFIG_GLOBAL":os.devnull,"GIT_CONFIG_SYSTEM":os.devnull,"GIT_TERMINAL_PROMPT":"0","GIT_OPTIONAL_LOCKS":"0","GIT_ASKPASS":"","SSH_ASKPASS":""})
    return env
...
def _assert_private_root(root:Path):
    ...
    if _linklike(root):raise BrokerRuntimeError("broker private root cannot be link/junction")
    if os.name!="nt":
        import stat
        st=root.stat()
...
def materialize_base(source_worktree:Path,base_sha:str,run_id:str,root:Path|None=None)->Path:
    ...
    if target.exists() or _linklike(target):raise BrokerRuntimeError("private run directory already exists/replay")
