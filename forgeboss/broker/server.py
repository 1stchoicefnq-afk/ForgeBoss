...
from .reintegrate import build_and_handoff
...
def _authority_view(req):
    ...
    checks={"taskId":task,"runId":run,"ownerEpoch":epoch,"envelopeSha256":env_sha,"budgetUsd":budget,"worktreePath":str(Path(work).resolve()),"baseSha":base.lower(),"allowedPaths":allowed,"expiresAt":exp}
...
class BrokerServer:
    ...
    def handle(self,request):
        ...
        request_sha=_digest(request);view=_authority_view(request);protected=self.authority_verifier.verify({"envelopeSha256":view["envelopeSha256"],"taskId":view["taskId"],"runId":view["runId"],"ownerEpoch":view["ownerEpoch"],"baseSha":view["baseSha"],"budgetUsd":view["budgetUsd"],"expiresAt":view["expiresAt"],"allowedPaths":view["allowedPaths"],"worktreePath":view["worktreePath"]});key,consume_id=_consume(view,request_sha,protected.attestation_id);repo=None;rr=None
        try:
...
def serve_forever():
    s=BrokerServer();serve_windows(s) if os.name=="nt" else serve_linux(s)
