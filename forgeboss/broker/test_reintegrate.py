...
class ReintegrationTests(unittest.TestCase):
    ...
    def protected(self):return mock.patch.object(reintegrate,"broker_root",return_value=self.broker_root),mock.patch.object(reintegrate,"_assert_private_root",return_value=self.broker_root.resolve())
    ...
    def test_result_commit_and_ref_ignore_old_host_file_handle(self):
        (self.repo/"allowed.txt").write_text("protected-result\n",encoding="utf-8");commit,tree,paths,diff_sha=reintegrate.create_result_commit(self.repo,self.base,["allowed.txt"],"task","run");host=self.root/"ordinary-host.txt";host.write_text("old-host",encoding="utf-8");p1,p2=self.protected()
        with host.open("r+b",buffering=0) as held,p1,p2:
            result=reintegrate.handoff_result(self.repo,self.base,commit,tree,paths,diff_sha,"task","run");held.seek(0);held.write(b"HOSTILE!");held.flush();ref=subprocess.run(["git","--git-dir",str(self.broker_root/"handoff.git"),"show-ref","--verify","--hash",result.result_ref],capture_output=True,text=True,check=True).stdout.strip()
    ...
    def test_same_task_run_result_ref_cannot_be_reused(self):
        (self.repo/"allowed.txt").write_text("one\n",encoding="utf-8");commit,tree,paths,diff_sha=reintegrate.create_result_commit(self.repo,self.base,["allowed.txt"],"task","run");p1,p2=self.protected()
        with p1,p2:
