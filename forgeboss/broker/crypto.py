...
WIN_ROOT=Path(r"C:\ProgramData\ForgeBoss\IsolationBroker");LINUX_ROOT=Path("/var/lib/forgeboss/isolation-broker");WIN_TRUST_KEY=r"SOFTWARE\ForgeBoss\IsolationBroker";WIN_AUTH_PIPE=r"\\.\pipe\ForgeBossControllerAuthority.v1";LINUX_AUTH_SOCKET=Path("/run/forgeboss/controller-authority-v1.sock");LINUX_AUTH_TRUST=Path("/etc/forgeboss/controller-authority.json")
def broker_root()->Path:return WIN_ROOT if os.name=="nt" else LINUX_ROOT
...
def _read_protected(path:Path,root_owned=False)->bytes:
    try:
        if path.is_symlink() or not path.is_file():raise BrokerCryptoError("protected file is not regular: "+str(path))
        st=path.stat()
...
def load_controller_public_key()->bytes:
    ...
    try:return _b64key(json.loads(_read_protected(LINUX_AUTH_TRUST,True).decode())["ed25519PublicKeyBase64"],"controller public key")
    except BrokerCryptoError:raise
    except Exception as e:raise BrokerCryptoError("controller public key trust unavailable") from e
class ReceiptSigner:
