...
class LearningStore:
    def __init__(self,path:Path):
        ...
        self.db=sqlite3.connect(str(self.path),timeout=15); self.db.row_factory=sqlite3.Row
    ...
    def fingerprint(problem_family,evidence):
        blob=json.dumps({"family":problem_family,"evidence":evidence},sort_keys=True,separators=(",",":"))
        return hashlib.sha256(blob.encode()).hexdigest()
    def record_verified_lesson(self,project_id,problem_family,summary,repair_pattern,evidence,validation,confidence=1.0,reusable=True):
        fp=self.fingerprint(problem_family,evidence); lid=hashlib.sha256(f"{project_id}:{problem_family}:{fp}".encode()).hexdigest(); now=time.time()
        self.db.execute("""INSERT INTO lessons(lesson_id,project_id,fingerprint,problem_family,summary,repair_pattern_json,evidence_json,
          ...
          (lid,project_id,fp,problem_family,summary,json.dumps(repair_pattern),json.dumps(evidence),json.dumps(validation),float(confidence),1 if reusable else 0,now,now))
        self.db.commit(); return lid
    def find_reusable(self,project_id,problem_family,min_confidence=.85,limit=5):
        rows=self.db.execute("""SELECT * FROM lessons WHERE project_id=? AND problem_family=? AND reusable=1
          AND confidence>=? ORDER BY confidence DESC,success_count DESC,last_used_at DESC LIMIT ?""",
          (project_id,problem_family,float(min_confidence),int(limit))).fetchall()
        return [dict(r) for r in rows]
    def mark_reuse_result(self,lesson_id,success):
        if success:
            self.db.execute("UPDATE lessons SET success_count=success_count+1,last_used_at=? WHERE lesson_id=?",(time.time(),lesson_id))
        else:
            self.db.execute("UPDATE lessons SET failure_count=failure_count+1,confidence=MAX(0.0,confidence-0.15),last_used_at=? WHERE lesson_id=?",(time.time(),lesson_id))
        self.db.commit()
    def record_worker_result(self,project_id,worker_id,problem_family,success,cost_usd,latency_ms):
        self.db.execute("""INSERT INTO routing_stats(project_id,worker_id,problem_family,attempts,successes,total_cost,total_latency_ms)
          VALUES(?,?,?,?,?,?,?)
    ...
    def best_workers(self,project_id,problem_family):
        rows=self.db.execute("""SELECT *,CASE WHEN attempts>0 THEN CAST(successes AS REAL)/attempts ELSE 0 END success_rate,
          ...
          ORDER BY success_rate DESC,cost_per_success ASC,total_latency_ms/MAX(attempts,1) ASC""",(project_id,problem_family)).fetchall()
        return [dict(r) for r in rows]
