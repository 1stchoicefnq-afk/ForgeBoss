...
"""
 with tempfile.TemporaryDirectory() as td:
  ...
  prelude=f"""$Root={_psq(fixture_root)}
...
$repairDir=Join-Path $Root 'state\\repair-rat'
$reviewDir=Join-Path $Root 'state\\rat-review'
[void][IO.Directory]::CreateDirectory($repairDir)
[void][IO.Directory]::CreateDirectory($reviewDir)
function Write-GateCost([string]$Path,[string]$GeneratedAt,[double]$Usd) {{
 $json=[ordered]@{{generated_at=$GeneratedAt;usage_estimated_usd=$Usd}}|ConvertTo-Json -Compress
 [IO.File]::WriteAllText($Path,$json,[Text.UTF8Encoding]::new($false))
}}
Write-GateCost (Join-Path $repairDir 'today-repair.json') ($script:fbGateToday+'T00:00:00Z') $script:fbGateA
Write-GateCost (Join-Path $reviewDir 'today-review.json') ($script:fbGateToday+'T23:59:59Z') $script:fbGateB
Write-GateCost (Join-Path $repairDir 'prior-day-repair.json') ($script:fbGateOld+'T23:59:59Z') 999999999
Write-GateCost (Join-Path $repairDir 'malformed.json') 'not-a-timestamp' 777777777
"""
