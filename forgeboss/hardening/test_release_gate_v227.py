...
"""
 ...
 @unittest.skipUnless(gate.find_powershell(),"PowerShell runtime unavailable")
 def test_utc_rollover_executes_live_get_today_cost_against_real_files(self):
...
"""
 ...
 @unittest.skipUnless(gate.find_powershell(),"PowerShell runtime unavailable")
 def test_utc_function_that_counts_every_json_is_rejected(self):
  text="""function Get-TodayCost {
  $today=[DateTimeOffset]::UtcNow.ToString('yyyy-MM-dd')
  $total=0.0
  foreach($dir in @((Join-Path $Root 'state\\repair-rat'),(Join-Path $Root 'state\\rat-review'))){
   if(-not(Test-Path -LiteralPath $dir)){continue}
   foreach($f in @(Get-ChildItem -LiteralPath $dir -File -Filter '*.json' -ErrorAction SilentlyContinue)){
    try{$j=Get-Content -LiteralPath $f.FullName -Raw|ConvertFrom-Json;if($null-ne$j.usage_estimated_usd){$total+=[double]$j.usage_estimated_usd}}catch{}
   }
  }
  return $total
}
"""
 ...
 @unittest.skipUnless(gate.find_powershell(),"PowerShell runtime unavailable")
 def test_utc_function_that_ignores_review_cost_is_rejected(self):
  text="""function Get-TodayCost {
  $today=[DateTimeOffset]::UtcNow.ToString('yyyy-MM-dd')
  $total=0.0
  $dir=Join-Path $Root 'state\\repair-rat'
  foreach($f in @(Get-ChildItem -LiteralPath $dir -File -Filter '*.json' -ErrorAction SilentlyContinue)){
   try{$j=Get-Content -LiteralPath $f.FullName -Raw|ConvertFrom-Json;$stamp=\"$($j.generated_at)\";if($stamp.StartsWith($today)-and$null-ne$j.usage_estimated_usd){$total+=[double]$j.usage_estimated_usd}}catch{}
  }
  return $total
}
"""
