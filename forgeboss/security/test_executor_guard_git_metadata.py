...
class ExecutorGuardGitMetadataTests(unittest.TestCase):
    ...
    def _verify(self,work,lease):
        token="test-token"
        packet_path=work/"verify-packet.json";packet_path.write_text(json.dumps({"allowed_files":["allowed.txt"],"context_files":[]}),encoding="utf-8")
        full=dict(lease);full.update({"executor":"mini-swe","workspace":str(work.resolve()),"packet_sha256":guard.phash(packet_path),"expires_at":time.time()+60,"token_sha256":hashlib.sha256(token.encode()).hexdigest()})
        lease_path=work/"verify-lease.json";lease_path.write_text(json.dumps(full),encoding="utf-8")
        with patch.object(guard,"git",side_effect=self._real_guard_git),patch.object(guard,"no_remotes"):
            return guard.verify(lease_path,token,packet_path,work,"mini-swe")
    ...
    def test_external_core_hookspath_is_denied_before_lease_even_if_config_text_unchanged(self):
        td,work=self._ordinary()
        try:
            external=work.parent/"external-hooks";external.mkdir();hook=external/"pre-commit";hook.write_text("#!/bin/sh\nexit 0\n",encoding="utf-8");self._git(work,"config","core.hooksPath",str(external))
            with patch.object(guard,"git",side_effect=self._real_guard_git):
    ...
    def test_credential_helper_is_denied_even_if_external_target_changes(self):
        td,work=self._ordinary()
        try:
            external=work.parent/"cred-helper.sh";external.write_text("#!/bin/sh\nexit 0\n",encoding="utf-8");self._git(work,"config","credential.helper",external.as_posix())
            with patch.object(guard,"git",side_effect=self._real_guard_git):
                with self.assertRaisesRegex(guard.SecurityError,"execution-capable Git config denied: credential.helper"):guard.git_metadata_snapshot(work)
            external.write_text("#!/bin/sh\nexit 1\n",encoding="utf-8")
            with patch.object(guard,"git",side_effect=self._real_guard_git):
                with self.assertRaisesRegex(guard.SecurityError,"execution-capable Git config denied: credential.helper"):guard.git_metadata_snapshot(work)
        finally:td.cleanup()
    ...
    def test_external_core_worktree_redirection_is_denied(self):
        td,work=self._ordinary()
        try:
            external=work.parent/"external-worktree";external.mkdir();self._git(work,"config","core.worktree",str(external))
            with patch.object(guard,"git",side_effect=self._real_guard_git):
    ...
    def test_verify_unchanged_git_metadata_passes(self):
        td,work=self._ordinary()
        try:self.assertEqual(self._verify(work,self._lease(work))["workspace"],str(work.resolve()))
        finally:td.cleanup()

    def test_verify_rejects_post_lease_core_worktree_redirect(self):
        td,work=self._ordinary()
        try:
            lease=self._lease(work);external=work.parent/"redirected-worktree";external.mkdir();self._git(work,"config","core.worktree",str(external))
            with self.assertRaisesRegex(guard.SecurityError,"effective Git worktree escapes leased workspace"):self._verify(work,lease)
        finally:td.cleanup()

    def test_verify_rejects_post_lease_config_drift(self):
        td,work=self._ordinary()
        try:
            lease=self._lease(work);self._git(work,"config","core.filemode","false")
            with self.assertRaisesRegex(guard.SecurityError,"Git metadata changed before executor start"):self._verify(work,lease)
        finally:td.cleanup()

    def test_verify_rejects_post_lease_protocol_escape(self):
        td,work=self._ordinary()
        try:
    ...
    def test_verify_rejects_linked_worktree_metadata_drift(self):
        td,repo,work=self._linked()
        try:
            lease=self._lease(work);self._git(repo,"config","core.filemode","false")
            with self.assertRaisesRegex(guard.SecurityError,"Git metadata changed before executor start"):self._verify(work,lease)
    ...
    def test_verify_rejects_missing_git_metadata_baseline(self):
        td,work=self._ordinary()
        try:
            lease=self._lease(work);lease.pop("git_metadata")
            with self.assertRaisesRegex(guard.SecurityError,"executor lease missing Git metadata baseline"):self._verify(work,lease)
        finally:td.cleanup()

    def test_verify_benign_stat_cache_churn_does_not_false_fail(self):
        td,repo,work=self._linked()
        try:
            lease=self._lease(work);self._git(work,"status","--porcelain");self._verify(work,lease)
        finally:td.cleanup()
