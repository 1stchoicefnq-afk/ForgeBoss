...
class ExecutorGuardGitMetadataTests(unittest.TestCase):
    ...
    def test_credential_helper_is_denied_even_if_external_target_changes(self):
        ...
        try:
            ...
            self._git(work,"config","credential.helper",external.as_posix())
            with patch.object(guard,"git",side_effect=self._real_guard_git):
                with self.assertRaisesRegex(guard.SecurityError,"execution-capable Git config denied: credential.helper"):
                    guard.git_metadata_snapshot(work)
            external.write_text("#!/bin/sh\nexit 1\n",encoding="utf-8")
            with patch.object(guard,"git",side_effect=self._real_guard_git):
                with self.assertRaisesRegex(guard.SecurityError,"execution-capable Git config denied: credential.helper"):
    ...
    def test_diff_external_is_denied_fail_closed(self):
        ...
        try:
            ...
            self._git(work,"config","diff.external",external.as_posix())
            with patch.object(guard,"git",side_effect=self._real_guard_git):
                with self.assertRaisesRegex(guard.SecurityError,"execution-capable Git config denied: diff.external"):
                    guard.git_metadata_snapshot(work)
        finally:td.cleanup()
    ...
    def test_diff_textconv_is_denied_fail_closed(self):
        ...
        try:
            ...
            self._git(work,"config","diff.bin.textconv",external.as_posix())
            with patch.object(guard,"git",side_effect=self._real_guard_git):
                with self.assertRaisesRegex(guard.SecurityError,"execution-capable Git config denied: diff.bin.textconv"):
                    guard.git_metadata_snapshot(work)
        finally:td.cleanup()
        ...
        for name in ("core.editor","core.askPass","sequence.editor","core.sshCommand","gpg.program"):
            with self.subTest(name=name):
                ...
                try:
                    ...
                    with patch.object(guard,"git",side_effect=self._real_guard_git):
                        with self.assertRaisesRegex(guard.SecurityError,"execution-capable Git config denied"):
                            guard.git_metadata_snapshot(work)
                finally:td.cleanup()
