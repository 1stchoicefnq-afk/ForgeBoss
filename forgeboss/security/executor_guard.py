...
# Config keys that directly launch programs/commands are not safe to trust from
# config text alone because the referenced executable can mutate after lease issue.
EXEC_CONFIG_EXACT={
...
def _assert_safe_execution_config(work:Path,gitdir:Path,common:Path):
    ...
    # core.hooksPath redirects Git to executable hooks. Only the ordinary
    # snapshotted Git hook roots are trusted; any other configured target is denied.
    if "core.hookspath" in names:
        ...
        if target not in trusted:
            raise SecurityError("external Git execution target denied: core.hooksPath -> "+str(target))
    # Path-valued fsmonitor hooks execute outside Git's metadata model. Boolean
    # built-in enable/disable values are not external executable targets.
    if "core.fsmonitor" in names:
        ...
        if any(v not in ("true","false") for v in vals):
            raise SecurityError("external Git execution target denied: core.fsmonitor")
    # Centralized fail-closed policy for config that directly launches external
    # commands/programs. Hashing config text cannot bind those executable bytes.
    for name in names:
        if name in EXEC_CONFIG_EXACT or any(rx.match(name) for rx in EXEC_CONFIG_PATTERNS):
            raise SecurityError("execution-capable Git config denied: "+name)
        if name.startswith("alias."):
            vals=_config_values(work,name)
            if any(v.lstrip().startswith("!") for v in vals):
                raise SecurityError("execution-capable Git config denied: "+name)
...
def git_metadata_snapshot(work):
    ...
    gitdir=_resolve_git_dir(work,"--git-dir")
    common=_resolve_git_dir(work,"--git-common-dir")
    ...
    # Git follows include.path/includeIf and higher-scope config. Hash the effective
    # config with origin/scope so external dependency mutation cannot hide behind
    # unchanged local config bytes.
    ...
    # Compare semantic staged state rather than raw index bytes to avoid harmless
    # stat-cache refresh false positives from read-only Git commands.
