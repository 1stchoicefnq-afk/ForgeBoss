...
def _assert_effective_worktree(work:Path):
    try:
        ...
        effective=effective.resolve(strict=True)
        leased=work.resolve(strict=True)
    except SecurityError:raise
...
def _assert_safe_execution_config(work:Path,gitdir:Path,common:Path):
    _assert_effective_worktree(work)
...
def verify(lease_path,token,packet_path,workspace,executor):
    ...
    git_after=git_metadata_snapshot(work)
    git_ch=changed(git_before,git_after)
    if git_ch:raise SecurityError("Git metadata changed after lease before execution: "+json.dumps(git_ch))
    return lease
