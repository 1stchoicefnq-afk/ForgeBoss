...
def verify(lease_path,token,packet_path,workspace,executor):
    ...
    git_now=git_metadata_snapshot(work);git_ch=changed(git_before,git_now)
    if git_ch:raise SecurityError("Git metadata changed before executor start: "+json.dumps(git_ch))
