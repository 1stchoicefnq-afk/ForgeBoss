...
EXEC_CONFIG_PATTERNS=(
    ...
    re.compile(r"^diff\..+\.command$",re.I),
...
def _assert_safe_execution_config(work:Path,gitdir:Path,common:Path):
    ...
    # These config families are command strings, not passive values. Their target
    # identity/content cannot be safely bound by hashing config text alone.
    for name in names:
        if any(rx.match(name) for rx in EXEC_CONFIG_PATTERNS):
