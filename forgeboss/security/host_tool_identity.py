...
def _identity_from_path(name: str, candidate: Path) -> HostToolIdentity:
    if is_linklike(candidate):
        raise HostToolIdentityError("linklike host executable denied")
    try:
        path = candidate.resolve(strict=True)
    except OSError as ex:
        raise HostToolIdentityError(f"host executable unavailable: {name}") from ex
    if not path.is_file():
    ...
    if before_stat.st_size != after_stat.st_size or getattr(before_stat, "st_mtime_ns", None) != getattr(after_stat, "st_mtime_ns", None):
        raise HostToolIdentityError("host executable metadata changed while hashing")
    return HostToolIdentity(name=name,path=str(path),sha256=before,size=int(after_stat.st_size))
...
def resolve_trusted_host_executable(name: str) -> HostToolIdentity:
    ...
    return _identity_from_path(name, Path(found))


def trust_explicit_host_executable(name: str, path: str | os.PathLike[str]) -> HostToolIdentity:
    if not isinstance(name, str) or not name or any(ch in name for ch in "\x00\r\n/\\"):
        raise HostToolIdentityError("host tool name is invalid")
    return _identity_from_path(name, Path(path))
