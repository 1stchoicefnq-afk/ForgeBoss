...
def _is_reparse(path: Path) -> bool:
    try:
        ...
        raise AuthorityError('PROTECTED_ROOT_ANCESTOR_INVALID') from exc
...
def _service_uid(boundary) -> int:
    ...
    if not expected.startswith('uid:') or not expected[4:].isdigit():
        raise AuthorityError('SERVICE_PRINCIPAL_INVALID')
    return int(expected[4:])


def _object_ace_sid_offset(object_flags: int, ace_size: int) -> int:
    """Return a validated SID offset for object/callback-object ACEs."""
    if not isinstance(object_flags, int) or object_flags < 0 or object_flags & ~0x3:
        raise AuthorityError('PROTECTED_ROOT_ANCESTOR_ACL_UNKNOWN')
    off = 12 + (16 if object_flags & 1 else 0) + (16 if object_flags & 2 else 0)
    if off >= int(ace_size):
        raise AuthorityError('PROTECTED_ROOT_ANCESTOR_ACL_UNKNOWN')
    return off
...
def _windows_current_object_allows(path: Path):
    ...
    if rc != 0 or not owner.value or not dacl.value:
        raise AuthorityError('PROTECTED_ROOT_ANCESTOR_PERMISSIONS')
    try:
        ...
        for i in range(int(info.AceCount)):
            ace = V()
            if not api.advapi32.GetAce(dacl, i, ctypes.byref(ace)) or not ace.value:
                raise AuthorityError('PROTECTED_ROOT_ANCESTOR_PERMISSIONS')
            h = ctypes.cast(ace, ctypes.POINTER(ACEHDR)).contents
            size = int(h.AceSize); ace_type = int(h.AceType); flags = int(h.AceFlags); base = int(ace.value)
            if size < 8:
                raise AuthorityError('PROTECTED_ROOT_ANCESTOR_ACL_UNKNOWN')
            ...
            if ace_type in (0, 9):
                ...
                if size < 12:
                    raise AuthorityError('PROTECTED_ROOT_ANCESTOR_ACL_UNKNOWN')
                object_flags = int(ctypes.c_uint32.from_address(base + 8).value)
                off = _object_ace_sid_offset(object_flags, size)
            elif ace_type in (1, 2, 3, 4, 6, 7, 8, 10, 12, 13, 14, 15, 16, 17):
                # Deny/audit/mandatory/resource ACEs do not add authority. Deny
                # masks are deliberately not subtracted from an allow mask.
                continue
            else:
                raise AuthorityError('PROTECTED_ROOT_ANCESTOR_ACL_UNKNOWN')
            if off >= size:
                raise AuthorityError('PROTECTED_ROOT_ANCESTOR_ACL_UNKNOWN')
            sid = _windows_sid_text(V(base + off)).casefold()
            mask = int(ctypes.c_uint32.from_address(base + 4).value)
            allows[sid] = allows.get(sid, 0) | mask
...
def _assert_windows_ancestor(boundary, path: Path) -> None:
    if not hasattr(boundary, 'trusted_storage'):
        boundary.assert_protected_path(path, protected_root=path)
        return
    ...
    for sid, mask in rights.items():
        if sid not in trusted and mask & _WIN_ANCESTOR_REPLACE_RIGHTS:
            raise AuthorityError('PROTECTED_ROOT_ANCESTOR_PERMISSIONS')


def assert_machine_anchored_root(boundary, protected_root: Path) -> Path:
    root = Path(protected_root)
    if not root.is_absolute() or root.is_symlink() or _is_reparse(root):
        raise AuthorityError('PROTECTED_ROOT_ANCESTOR_INVALID')
    try:
    ...
    if os.name != 'nt':
        ...
        for component in chain:
            ...
            if component == resolved:
                if st.st_uid != uid or mode & 0o077:
                    raise AuthorityError('PROTECTED_ROOT_ANCESTOR_PERMISSIONS')
            elif st.st_uid not in {0, uid} or mode & 0o022:
                raise AuthorityError('PROTECTED_ROOT_ANCESTOR_PERMISSIONS')
        if chain[-1] != Path('/'):
            raise AuthorityError('PROTECTED_ROOT_ANCESTOR_INVALID')
    else:
        anchor = Path(resolved.anchor)
        if not anchor or chain[-1] != anchor:
            raise AuthorityError('PROTECTED_ROOT_ANCESTOR_INVALID')
        for component in chain:
            if component.is_symlink() or _is_reparse(component):
                raise AuthorityError('PROTECTED_ROOT_ANCESTOR_INVALID')
            if component != resolved:
                _assert_windows_ancestor(boundary, component)
    return resolved
