...
OPERATIONS={'read_github_control','publish_report_comment','publish_reviewed_draft_pr','verify_launch_authority','prepare_self_build','prepare_self_build_replacement','self_build_status','revoke_self_build_worker','complete_self_build_worker'}
_REPO=re.compile(r'^[A-Za-z0-9][A-Za-z0-9._-]{0,99}/[A-Za-z0-9][A-Za-z0-9._-]{0,99}_PEER=re.compile(r'^[A-Za-z0-9][A-Za-z0-9._:@/-]{0,127}$');_REF=re.compile(r'^[A-Za-z0-9](?:[A-Za-z0-9._/-]{0,199})$');_SECRET_KEY=re.compile(r'(?:token|secret|private[_-]?key|pem|jwt|credential|password)',re.I);_RUN_ID=re.compile(r'^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$')
...
def _payload(op,raw):
    ...
    if op=='prepare_self_build':
        ...
        return o
    if op in {'prepare_self_build_replacement','self_build_status'}:
    ...
    if op=='revoke_self_build_worker':
        ...
        return o
    if op=='complete_self_build_worker':
        o=_exact(raw,('runId','taskId','workerRunId','ownerEpoch','resultHead','measuredCostUsd','resultDigest'),'PAYLOAD_INVALID')
        o['runId']=_text(o['runId'],'RUN_ID_INVALID',64,_RUN_ID)
        o['taskId']=_text(o['taskId'],'TASK_ID_INVALID',128,_PEER)
        o['workerRunId']=_text(o['workerRunId'],'WORKER_RUN_ID_INVALID',128,_PEER)
        o['ownerEpoch']=_int(o['ownerEpoch'],'OWNER_EPOCH_INVALID')
        o['resultHead']=_text(o['resultHead'],'RESULT_HEAD_INVALID',64,_OID,True)
        o['measuredCostUsd']=_text(o['measuredCostUsd'],'MEASURED_COST_INVALID',32,_MONEY)
        o['resultDigest']=_text(o['resultDigest'],'RESULT_DIGEST_INVALID',64,_HEX64,True)
        return o
...
);_HEX64=re.compile(r'^[0-9a-f]{64}_PEER=re.compile(r'^[A-Za-z0-9][A-Za-z0-9._:@/-]{0,127}$');_REF=re.compile(r'^[A-Za-z0-9](?:[A-Za-z0-9._/-]{0,199})$');_SECRET_KEY=re.compile(r'(?:token|secret|private[_-]?key|pem|jwt|credential|password)',re.I);_RUN_ID=re.compile(r'^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$')
class AuthorityError(RuntimeError):
    def __init__(self,code:str,message:str='protected authority request denied'):super().__init__(message);self.code=code
    ...
    def walk(n):
        if isinstance(n,Mapping):
            ...
            for s in texts:
                if s and s in n:raise AuthorityError('SECRET_VALUE_DENIED')
    walk(clean);return clean
);_OID=re.compile(r'^(?:[0-9a-f]{40}|[0-9a-f]{64})_PEER=re.compile(r'^[A-Za-z0-9][A-Za-z0-9._:@/-]{0,127}$');_REF=re.compile(r'^[A-Za-z0-9](?:[A-Za-z0-9._/-]{0,199})$');_SECRET_KEY=re.compile(r'(?:token|secret|private[_-]?key|pem|jwt|credential|password)',re.I);_RUN_ID=re.compile(r'^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$')
class AuthorityError(RuntimeError):
    def __init__(self,code:str,message:str='protected authority request denied'):super().__init__(message);self.code=code
    ...
    def walk(n):
        if isinstance(n,Mapping):
            ...
            for s in texts:
                if s and s in n:raise AuthorityError('SECRET_VALUE_DENIED')
    walk(clean);return clean
);_MONEY=re.compile(r'^[0-9]{1,6}(?:\.[0-9]{1,6})?_PEER=re.compile(r'^[A-Za-z0-9][A-Za-z0-9._:@/-]{0,127}$');_REF=re.compile(r'^[A-Za-z0-9](?:[A-Za-z0-9._/-]{0,199})$');_SECRET_KEY=re.compile(r'(?:token|secret|private[_-]?key|pem|jwt|credential|password)',re.I);_RUN_ID=re.compile(r'^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$')
class AuthorityError(RuntimeError):
    def __init__(self,code:str,message:str='protected authority request denied'):super().__init__(message);self.code=code
    ...
    def walk(n):
        if isinstance(n,Mapping):
            ...
            for s in texts:
                if s and s in n:raise AuthorityError('SECRET_VALUE_DENIED')
    walk(clean);return clean
);_PEER=re.compile(r'^[A-Za-z0-9][A-Za-z0-9._:@/-]{0,127}$');_REF=re.compile(r'^[A-Za-z0-9](?:[A-Za-z0-9._/-]{0,199})$');_SECRET_KEY=re.compile(r'(?:token|secret|private[_-]?key|pem|jwt|credential|password)',re.I);_RUN_ID=re.compile(r'^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$')
class AuthorityError(RuntimeError):
    def __init__(self,code:str,message:str='protected authority request denied'):super().__init__(message);self.code=code
