...
def serve_unix_once(
    ...
    *,
    ...
    preauth_timeout: float | None = None,
    stop_event=None,
    on_connection=None,
    allowed_peer_principals=None,
) -> bytes:
    if os.name == 'nt':
        raise AuthorityError('IPC_PLATFORM_INVALID')
    if accept_timeout is not None:
        if not isinstance(accept_timeout, (int, float)) or not 0.01 <= float(accept_timeout) <= 5.0:
            raise AuthorityError('IPC_TIMEOUT_INVALID')
        listener.settimeout(float(accept_timeout))
    try:
        conn, _ = listener.accept()
    except socket.timeout as exc:
        raise AuthorityError('IPC_ACCEPT_TIMEOUT') from exc
    except OSError as exc:
        if stop_event is not None and stop_event.is_set():
            raise AuthorityError('IPC_STOPPED') from exc
        raise AuthorityError('IPC_ACCEPT_FAILED') from exc
    if on_connection is not None:
        on_connection(conn, True)
    try:
        ctx = unix_peer_context(conn)
        if allowed_peer_principals is not None:
            allowed = {str(x).casefold() for x in allowed_peer_principals}
            if not allowed or ctx.principal.casefold() not in allowed:
                raise AuthorityError('PEER_CONTEXT_DENIED')
        chunks = []
        total = 0
        deadline = None
        ...
        while True:
            ...
            try:
                ...
                if stop_event is not None and stop_event.is_set():
                    raise AuthorityError('IPC_STOPPED') from exc
                raise AuthorityError('IPC_READ_FAILED') from exc
            if not part:
                break
            total += len(part)
        ...
        if not total:
            raise AuthorityError('REQUEST_SIZE_INVALID')
        result = service.handle_json(b''.join(chunks), peer_context=ctx)
        conn.sendall(result)
        return result
    finally:
        if on_connection is not None:
            on_connection(conn, False)
        try:
            conn.close()
        except OSError:
            pass
