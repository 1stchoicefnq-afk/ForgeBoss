...
class _Service:
    def __init__(self):self.raw=None;self.ctx=None
    def handle_json(self,raw,*,peer_context):self.raw=raw;self.ctx=peer_context;return b'{}'
...
class ProtectedAuthorityWin32V4Tests(unittest.TestCase):
    ...
    def _server(self,td,service=None):return WindowsNamedPipeServer(service=service or _Service(),boundary=_Boundary(),protected_root=Path(td),allowed_peer_sids={'S-1-1-0'})
    ...
    def test_native_named_pipe_peer_context_path(self):
        with tempfile.TemporaryDirectory() as td:
            api=load_win32();service=_Service();server=self._server(td,service);server.start();result={}
            def serve():
                try:result['out']=server.serve_once()
            ...
            try:
                ...
                client=api.kernel32.CreateFileW(FIXED_PIPE_NAME,0xC0000000,0,None,3,0,None);self.assertFalse(is_invalid_handle(client))
                try:
                    sent=ctypes.create_string_buffer(b'x');written=wintypes.DWORD(0);self.assertTrue(api.kernel32.WriteFile(client,sent,1,ctypes.byref(written),None));self.assertEqual(written.value,1)
                    reply=ctypes.create_string_buffer(2);read=wintypes.DWORD(0);self.assertTrue(api.kernel32.ReadFile(client,reply,2,ctypes.byref(read),None));self.assertEqual(reply.raw[:read.value],b'{}')
                    t.join(5);self.assertFalse(t.is_alive());self.assertNotIn('error',result);self.assertEqual(service.raw,b'x');self.assertEqual(service.ctx.platform,'windows');self.assertTrue(service.ctx.principal.startswith('s-1-'))
                finally:api.kernel32.CloseHandle(client)
            finally:server.close();t.join(1)
