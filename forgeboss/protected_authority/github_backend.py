...
class GitHubAppBackend:
    ...
    def _jwt(self,pem:bytes)->str:
        try:
            ...
            unsigned=enc(header)+b'.'+enc(payload);key=serialization.load_pem_private_key(pem,password=None);sig=key.sign(unsigned,padding.PKCS1v15(),hashes.SHA256());return (unsigned+b'.'+base64.urlsafe_b64encode(sig).rstrip(b'=')).decode()
    ...
    def _request(self,method:str,path:str,*,token:str|None=None,body:Mapping[str,Any]|None=None)->Any:
        ...
        data=None if body is None else json.dumps(body,separators=(',',':')).encode();req=urllib.request.Request(self.api_base+path,data=data,headers=headers,method=method)
        try:
    ...
    def _token(self,pem:bytes,repository:str,operation:str)->str:
        ...
        if perms is None:raise AuthorityError('GITHUB_TOKEN_SCOPE_DENIED')
        owner,sep,name=repository.partition('/')
        if not sep or not owner or not name:raise AuthorityError('GITHUB_REPOSITORY_INVALID')
    ...
    def read_github_control(self,*,repository:str,control_revision:int,payload:Mapping[str,Any],private_key:bytes)->Any:
        token=self._token(private_key,repository,'read_github_control');root=int(payload['rootPr']);preferred=int(payload['preferredRepairPr'])
        root_pr=self._request('GET',self._repo_path(repository)+f'/pulls/{root}',token=token);repair=None
    ...
    def publish_report_comment(self,*,repository:str,control_revision:int,payload:Mapping[str,Any],private_key:bytes)->Any:
        token=self._token(private_key,repository,'publish_report_comment');obj=self._request('POST',self._repo_path(repository)+f"/issues/{int(payload['issue'])}/comments",token=token,body={'body':payload['body']});return {'repository':repository,'controlRevision':control_revision,'commentId':obj.get('id'),'reportDigest':payload['reportDigest']}
    def publish_reviewed_draft_pr(self,*,repository:str,control_revision:int,payload:Mapping[str,Any],private_key:bytes)->Any:
        ...
        obj=self._request('POST',self._repo_path(repository)+'/pulls',token=token,body={'title':payload['title'],'body':payload['body'],'head':head_ref,'base':base_ref,'draft':True});return {'repository':repository,'controlRevision':control_revision,'prNumber':obj.get('number'),'draft':obj.get('draft') is True,'headSha':payload['headSha'],'baseSha':payload['baseSha'],'reviewDigest':payload['reviewDigest']}
