...
class WindowsRootChainV7Tests(unittest.TestCase):
    ...
    def test_standard_volume_root_allows_genuinely_protected_child(self):
        boundary,sid=self._boundary();volume=Path(os.environ.get('SystemDrive','C:')+'\\');root=volume/f'ForgeBossAuthorityV7-{uuid.uuid4().hex}'
        try:
            try:root.mkdir()
            except PermissionError:self.skipTest('runner cannot create direct system-volume fixture')
            self._protect(root,sid);self._assert_leaf_fixture(boundary,root);self.assertEqual(assert_machine_anchored_root(boundary,root),root.resolve())
        finally:__import__('shutil').rmtree(root,ignore_errors=True)
