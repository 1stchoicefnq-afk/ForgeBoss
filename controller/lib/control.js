...
function parseControlResult(stdout,invocationId){
 ...
 let raw,obj;
 try{raw=Buffer.from(lines[0].slice(CONTROL_MARKER.length),'base64');obj=JSON.parse(raw.toString('utf8'));}catch(e){fail(`authoritative control producer result invalid: ${e.message}`,'CONTROL_RESULT_INVALID');}
...
function readControl(root,cfg,stateRoot){
 ...
 const artifactDir=path.join(stateRoot,'control-artifacts');fs.mkdirSync(artifactDir,{recursive:true});
 ...
 let disk;try{disk=fs.readFileSync(out);}catch(e){fail(`authoritative control diagnostic artifact missing: ${e.message}`,'CONTROL_ARTIFACT_MISSING');}
...
module.exports={readControl,selectRepairTarget,_test:{parseControlResult,validateControlResult,sha256,CONTROL_MARKER}};
