...
process.env.FORGEBOSS_SENTINEL_SECRET='must-not-leak';process.env.OPENAI_API_KEY='allowed-provider-secret';const env=controlTest.minimalPowerShellEnv(trusted,'Ym91bmQ=');ok('minimal PowerShell env excludes unrelated controller secrets but preserves required runtime keys',()=>env.FORGEBOSS_SENTINEL_SECRET===undefined&&env.OPENAI_API_KEY==='allowed-provider-secret'&&env.FORGEBOSS_BOUND_ARGS_B64==='Ym91bmQ='&&env.PATH.startsWith(path.dirname(path.resolve(trusted))));delete process.env.FORGEBOSS_SENTINEL_SECRET;delete process.env.OPENAI_API_KEY;
const controlEnv=controlTest.minimalPowerShellEnv(trusted);ok('control-read PowerShell env does not receive provider credentials',()=>controlEnv.OPENAI_API_KEY===undefined&&controlEnv.ANTHROPIC_API_KEY===undefined&&controlEnv.FORGEBOSS_BOUND_ARGS_B64===undefined);
